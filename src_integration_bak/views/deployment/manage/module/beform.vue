<template>
  <el-dialog :append-to-body="true" :visible.sync="dialog" :title="isAdd ? '新增模块' : '编辑模块'" width="550px">
    <el-form ref="beform" :model="form" :rules="rules" size="small" label-width="130px">
      ...
      <!--
      <el-form-item label="开发负责人" prop="project_owner">
        <el-input v-model="form.project_owner" style="width: 370px;"/>
      </el-form-item>
      <el-form-item label="测试负责人" prop="project_tester">
        <el-input v-model="form.project_tester" style="width: 370px;"/>
      </el-form-item>
      <el-form-item label="开发leader" prop="project_leader">
        <el-input v-model="form.project_leader" style="width: 370px;"/>
      </el-form-item>
      -->
    </el-form>
...
<script>
import { add, edit } from '@/api/beproject'
...
export default {
  ...
  methods: {
    ...
    doSubmit() {
      this.$refs['beform'].validate((valid) => {
        if (valid) {
          this.loading = true
          if (this.isAdd) {
            this.doAdd()
