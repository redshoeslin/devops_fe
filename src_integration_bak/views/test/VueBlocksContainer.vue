...
export default {
  ...
  watch: {
    ...
    lines() {
      for (const link of this.links) {
        const originBlock = this.blocks.find(block => {
  ...
  methods: {
    ...
    /** @param e {MouseEvent} */
    handleMove(e) {
      const mouse = mouseHelper.getMousePosition(this.$el, e)
      this.mouseX = mouse.x
      this.mouseY = mouse.y
    ...
    linkingStop(targetBlock, slotNumber) {
      if (this.linkStartData && targetBlock && slotNumber > -1) {
        ...
        const maxID = Math.max(0, ...this.links.map(function(o) {
          return o.id
        }))
    ...
    linkingBreak(targetBlock, slotNumber) {
      if (targetBlock && slotNumber > -1) {
        ...
        if (findLink) {
          const findBlock = this.blocks.find(value => {
    ...
    addNewBlock(nodeName, x, y) {
      const maxID = Math.max(0, ...this.blocks.map(function(o) {
        return o.id
      }))
    ...
    prepareBlocksLinking(blocks, links) {
      ...
      blocks.forEach(block => {
        ...
        const outputs = links.filter(link => {
