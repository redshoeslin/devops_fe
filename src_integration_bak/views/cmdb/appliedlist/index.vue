  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" ref="table" :data="data" size="small" border style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="详情" width="150px" align="center">
        <template slot-scope="scope">
          <detail v-if="checkPermission(['admin','connection_all','connection_edit'])" :data="scope.row" :sup_this="sup_this" :service_types="service_types"/>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
...
export default {
  ...
  data() {
    return {
      delLoading: false, sup_this: this, service_types: []
    }
  },
  created() {
    this.$nextTick(() => {
